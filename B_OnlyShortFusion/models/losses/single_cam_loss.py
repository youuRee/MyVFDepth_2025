...
class SingleCamLoss(BaseLoss):
    ...
    def compute_reproj_loss(self, inputs, target_view, cam=0, scale=0, ref_mask=None):
        ...
        for frame_id in self.frame_ids[1:]:
            # pred: frame_id(-1 or 1)시점에서 0시점으로 와핑한 이미지
            reproj_loss_args = {
                'pred': target_view[('color', frame_id, scale)] * tar_gt_depth_mask,
                'target': inputs['color',0, 0][:, cam, ...] * tar_gt_depth_mask
            }
        ...
        for frame_id in self.frame_ids[1:]:
            # pred: 실제 frame_id 시점의 원본 이미지
            identity_reproj_loss_args = {
                'pred': inputs[('color', frame_id, 0)][:, cam, ...] * tar_gt_depth_mask,
                'target': inputs['color',0, 0][:, cam, ...] * tar_gt_depth_mask              
            }
        ...
        # find minimum losses
        # reprojection_loss가 identity_reprojection_loss 보다 작아야 target(0시점)을 잘 예측한 것 
        # identity_reprojection_loss가 더 작은 경우는 실제 자차가 안움직였을 가능성 높기(t-1 원본 이미지와 t 원본 이미지가 거의 같다는 의미이므로)때문에 무효처리 필요!
