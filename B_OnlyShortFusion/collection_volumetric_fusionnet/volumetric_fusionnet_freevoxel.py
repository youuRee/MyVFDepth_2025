...
class VFNet(nn.Module):
    ...
    def VoxelRayCasting(self, coords, grid_range, voxel_size, voxelwise):
        ...
        #free_voxel = torch.zeros(voxel_grid.shape).to('cuda')
        free_voxel = torch.zeros([1,64,20,100,100]).to('cuda')
        empty_idx = []
        empty_feat = None
        for i in range(coords.shape[0]):
            ray_origin = torch.tensor([0, 0, 0], dtype=torch.float32).to('cuda')
            v_z, v_y, v_x = coords[i]
            free_voxel[:, :, v_z, v_y, v_x] = voxelwise[i]  # 차있는 복셀 -> 2
            ...
            for j, point_coord in enumerate(sample_points_coords):
                ...
                if len(indices) == 0:  # 포인트가 없는 복셀 (empty voxel)
                    ...
                    free_voxel[:, :, z, y, x] = empty_feat
                    #free_voxel[:, :, z, y, x] = 1 # 비어있는 복셀 -> 1
                    #empty_idx.append([z.item(),y.item(),x.item()])
        
        
        unknown_idx = torch.nonzero(free_voxel==2)
        unknown_feat = MLP2(unknown_idx[0])
        free_voxel[:,:,unknown_idx] = unknown_feat
        

        # free_voxel에서 unknown_idx로 피처 할당
        unknown_idx = torch.nonzero(free_voxel == 0)  # unknown 복셀의 인덱스 추출 (n, 4)로 추출됨: (batch, c, z, y, x)
        unknown_feat = self.unknownnet(torch.tensor([1.0], device='cuda'))  # MLP2를 통해 (z, y, x) 좌표에 대한 특징 생성
        # unknown_idx를 이용해 free_voxel에 피처 벡터화 할당
        b, c, z, y, x = unknown_idx[:, 0], unknown_idx[:, 1], unknown_idx[:, 2], unknown_idx[:, 3], unknown_idx[:, 4]
        for idx in (unknown_idx):
            b, c, z, y, x = idx  # 인덱스에서 좌표 추출
            free_voxel[:, :, z, y, x] = unknown_feat  # 해당 좌표에 unknown_feat 값 할당
    ...
    def LiDARFeatureExtractor(self, lidar, free_voxel):
        ...
        #if self.model == 'depth':
            #voxel_feat = self.VoxelRayCasting(voxel_coordinates, grid_range, voxel_size, voxelwise)
            
        voxel_feat = torch.ones([1, 64, 20, 100, 100]).to('cuda')
        # 벡터화된 방식으로 차있는 복셀을 한 번에 할당
        v_z, v_y, v_x = coords[:, 0].long(), coords[:, 1].long(), coords[:, 2].long()
        voxel_feat[:, :, v_z, v_y, v_x] = voxelwise.T.unsqueeze(0)  # 차있는 복셀 -> voxelwise 값

        # unknown_idx를 벡터화된 방식으로 처리
        unknown_idx = torch.nonzero(free_voxel[0,0,:,:,:] == 0)
        unknown_feat = self.unknownnet(torch.tensor([1.0], device='cuda'))
        # 벡터화된 방식으로 unknown_feat 할당
        if unknown_idx.shape[0] > 0:
            z, y, x = unknown_idx[:, 0].long(), unknown_idx[:, 1].long(), unknown_idx[:, 2].long()
            voxel_feat[:, :, z, y, x] = unknown_feat.unsqueeze(0).unsqueeze(2).expand(1, 64, unknown_idx.shape[0])

        # empty
        empty_idx = torch.nonzero(free_voxel[0,0,:,:,:] == 0)
        empty_feat = self.emptynet(torch.tensor([-1.0], device='cuda'))
        if empty_idx.shape[0] > 0:
            z, y, x = empty_idx[:, 0].long(), empty_idx[:, 1].long(), empty_idx[:, 2].long()
            voxel_feat[:, :, z, y, x] = empty_feat.unsqueeze(0).unsqueeze(2).expand(1, 64, empty_idx.shape[0])
    ...
    def forward(self, inputs, feats_agg, frame_ids):
        ...
        if self.model == 'depth':
            ...
            free_voxel = inputs[('voxel', frame_ids)][:,0,:,:,:,:]
            lidar_voxel_feat = self.LiDARFeatureExtractor(pc[0][0], free_voxel)
            ...
            cur_free_voxel = inputs[('voxel', frame_ids[0])][:,0,:,:,:,:] 
            next_free_voxel = inputs[('voxel', frame_ids[1])][:,0,:,:,:,:]
            cur_voxel = self.LiDARFeatureExtractor(cur_pc[0][0], cur_free_voxel) # [1, 64, 20, 100, 100]
            next_voxel = self.LiDARFeatureExtractor(next_pc[0][0], next_free_voxel)
