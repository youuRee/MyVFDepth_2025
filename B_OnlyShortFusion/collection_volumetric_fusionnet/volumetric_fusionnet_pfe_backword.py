...
class VFNet(nn.Module):
    ...
    def __init__(self, cfg, feat_in_dim, feat_out_dim, model='depth'):
        ...
        self.pfe = PointFeatureExtractor(1, 32)
        #self.pfe = nn.Conv2d(in_channels=1, out_channels=32, kernel_size=1)
        self.vfe = VoxelFeatureExtractor(num_input_features=35, num_filters=[32, 64], voxel_size=(1, 1, 1.5), pc_range=(-50, -50, -15, 49, 49, 13.5))
        #self.vfe = nn.Conv1d(in_channels=5, out_channels=1, kernel_size=1)
        self.spconv_net = SpconvNet(self.sp_dim)
        #self.vfe = VoxelFeatureExtraction(3, 64)
    ...
    def project_voxel_into_image(self, cam_voxel_feat, lidar_voxel_feat, inv_K, extrinsics):
        ...
        voxel_feat = torch.cat((cam_voxel_feat, lidar_voxel_feat), dim=1)
        ...
        for cam in range(self.num_cams):
            ...
            proj_feat = proj_feat.view(b, self.proj_d_bins * (self.v_dim_o[-1] + self.sp_dim), self.img_h, self.img_w)
            #proj_feat = proj_feat.view(b, self.proj_d_bins * 64, self.img_h, self.img_w)
    ...
    def spconvTensor(self, feat):
        ...
        # feat:(b, c, 포인트 개수, 3)
        voxels, coords, num_points_in_voxel, pc_voxel_id = voxel_gen.generate_voxel_with_id(feat[0])
        '''
        ...
        voxels, coords, num_points_in_voxel = voxelize(feat[0], voxel_size, grid_range, max_points_in_voxel=5, max_num_voxels=200000)
        # voxels: (복셀개수, 5, 3), voxelwise: (복셀개수, 64)
        voxelwise = self.vfe(voxels, num_points_in_voxel, coords)#.clone().detach()
        #voxelwise = self.vfe(voxels) # (복셀개수, 1, 35)
        ...
        '''
        voxel_feat:  torch.Size([1, 257, 200000])
        voxels:  torch.Size([8306, 5, 3])
        voxelwise:  torch.Size([8306, 64])
        input_sp_tensor:  SparseConvTensor[shape=torch.Size([8306, 64])]
        '''
        # 포워드 패스를 수행하여 복셀 특징을 얻기
        output_sp_tensor = self.spconv_net(input_sp_tensor)
        output_dense_tensor = output_sp_tensor.dense()
        #print(output_dense_tensor.shape)
        #self.sp_voxel = output_dense_tensor
        return output_dense_tensor
    ...
    def forward(self, inputs, feats_agg, frame_ids):
        ...
        if self.model == 'depth':
            xyz = inputs[('lidar', frame_ids)][0][0] # (n,3)
            #intensity = inputs['intensity'][0][0]
            points = xyz.to('cpu').numpy()
            laser_scan = LaserScan(project=True, H=64, W=6858, fov_up=25.0, fov_down=-5.0)
            laser_scan.set_points(points=points[:, :3], remissions=None)

            range_image = laser_scan.proj_range # (64, 1715)
            range_image = torch.from_numpy(range_image).to('cuda').unsqueeze(0).unsqueeze(0) # (1, 1, 64, 1715)
            range_pc = laser_scan.proj_xyz # (64, 1715, 3)
            range_pc = torch.from_numpy(range_pc).to('cuda').permute(2,0,1).unsqueeze(0) # (1, 3, 64, 1715)
            range_mask = laser_scan.proj_mask # (64, 1715)
            range_mask = torch.from_numpy(range_mask).to('cuda').unsqueeze(0).unsqueeze(0) 

            range_feat = self.pfe(range_image)
            range_feat = torch.cat((range_pc, range_feat), dim=1)
            range_feat = range_feat[range_mask.bool().expand_as(range_feat)].view(1, 35, -1)
            '''
            pc = range_feat[:,:3]
            ch = range_feat[:,3,:].unsqueeze(0)

            # pc 텐서를 (1, 1, n, 3)로 확장
            pc = pc.transpose(1, 2).unsqueeze(1)  # (1, 1, n, 3)

            # ch 텐서를 (1, c, n, 1)로 확장
            ch = ch.unsqueeze(-1)  # (1, c, n, 1)

            # pc와 ch를 브로드캐스팅하여 (1, c, n, 3)으로 변환
            pts_feat = pc + ch  # (1, c, n, 3)
            '''
            lidar_voxel_feat = self.spconvTensor(range_feat.transpose(1, 2))
            ...
            cur_voxel = self.spconvTensor(cur_pc[0][0]) # [1, 64, 20, 100, 100]
            next_voxel = self.spconvTensor(next_pc[0][0])
            ...
            '''
            '''
            cur_pc = inputs[('lidar', frame_ids[0])][0][0] 
            cur_points = cur_pc.to('cpu').numpy()
            cur_laser_scan = LaserScan(project=True, H=64, W=6858, fov_up=25.0, fov_down=-5.0)
            cur_laser_scan.set_points(points=cur_points[:, :3], remissions=None)

            cur_range_image = cur_laser_scan.proj_range # (64, 1715)
            cur_range_image = torch.from_numpy(cur_range_image).to('cuda').unsqueeze(0).unsqueeze(0) # (1, 1, 64, 1715)
            cur_range_pc = cur_laser_scan.proj_xyz # (64, 1715, 3)
            cur_range_pc = torch.from_numpy(cur_range_pc).to('cuda').permute(2,0,1).unsqueeze(0) # (1, 3, 64, 1715)
            cur_range_mask = cur_laser_scan.proj_mask # (64, 1715)
            cur_range_mask = torch.from_numpy(cur_range_mask).to('cuda').unsqueeze(0).unsqueeze(0) 

            cur_range_feat = self.pfe(cur_range_image)
            cur_range_feat = torch.cat((cur_range_pc, cur_range_feat), dim=1)
            cur_range_feat = cur_range_feat[cur_range_mask.bool().expand_as(cur_range_feat)].view(1, 35, -1)
            cur_voxel = self.spconvTensor(cur_range_feat.transpose(1, 2))

            next_pc = inputs[('lidar', frame_ids[1])][0][0]
            next_points = next_pc.to('cpu').numpy()
            next_laser_scan = LaserScan(project=True, H=64, W=6858, fov_up=25.0, fov_down=-5.0)
            next_laser_scan.set_points(points=next_points[:, :3], remissions=None)

            next_range_image = next_laser_scan.proj_range # (64, 1715)
            next_range_image = torch.from_numpy(next_range_image).to('cuda').unsqueeze(0).unsqueeze(0) # (1, 1, 64, 1715)
            next_range_pc = next_laser_scan.proj_xyz # (64, 1715, 3)
            next_range_pc = torch.from_numpy(next_range_pc).to('cuda').permute(2,0,1).unsqueeze(0) # (1, 3, 64, 1715)
            next_range_mask = next_laser_scan.proj_mask # (64, 1715)
            next_range_mask = torch.from_numpy(next_range_mask).to('cuda').unsqueeze(0).unsqueeze(0) 

            next_range_feat = self.pfe(next_range_image)
            next_range_feat = torch.cat((next_range_pc, next_range_feat), dim=1)
            next_range_feat = next_range_feat[next_range_mask.bool().expand_as(next_range_feat)].view(1, 35, -1)
            next_voxel = self.spconvTensor(next_range_feat.transpose(1, 2))
