...
class VFNet(nn.Module):
    ...
    def __init__(self, cfg, feat_in_dim, feat_out_dim, model='depth'):
        ...
        self.vfe = VoxelFeatureExtractor(num_input_features=3, num_filters=[8, 16], 
                                         ...
                                         pc_range=([-50, -50, -15, 49, 49, 13.5]))#self.voxel_str_p + self.voxel_end_p))
        ...
        #self.pool3d = nn.MaxPool3d(kernel_size=4, stride=4)
        ...
        self.conv3d_block = nn.Sequential(
            ...
            #nn.Conv3d(32, 64, kernel_size=2, stride=2),  # 3x3x3 3D Convolution
            #nn.MaxPool3d(kernel_size=2, stride=2),
            #nn.BatchNorm3d(64),  # 3D Batch Normalization
            nn.ReLU()
        )
    ...
    def LiDARFeatureExtractor(self, lidars, voxels_label):
        
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        voxel_gen = PointToVoxel(vsize_xyz=self.lidar_voxel_unit_size,
                                coors_range_xyz=[-50, -50, -15, 49, 49, 13.5],#self.voxel_str_p + self.voxel_end_p,
        ...
        #check_label[:, v_z, v_y, v_x] = 1

        # empty
        empty_idx = torch.nonzero(voxels_label[0,0,:,:,:] == 1)
        ...
        voxel_label2 = voxels_label.detach().to('cpu').numpy()
        
        # 포인트 추출
        points = voxel_grid.reshape(3, -1).T  # [N, 3] 형식으로 변환
        labels = voxel_label2.reshape(-1)      # 라벨 [N] 형식으로 변환
        # unknown이 아닌 포인트만 선택
        mask = labels != 0                    # unknown (1=0)이 아닌 포인트에 대한 마스크 생성
        ...
        unknown_idx = torch.nonzero(voxels_label[0,:,:,:] == 0)
    ...
    def forward(self, inputs, feats_agg, frame_ids):
        ...
        if self.model == 'depth':
            ...
            lidar_voxel_feat = self.LiDARFeatureExtractor(pc, voxel_label)
            ...
            cur_voxel = self.LiDARFeatureExtractor(cur_pc, cur_voxel_label) # [b, 64, 20, 100, 100]
            next_voxel = self.LiDARFeatureExtractor(next_pc, next_voxel_label)
            ...
            lidar_voxel = torch.cat([cur_voxel, next_voxel], dim=1) # [1, 128, 200000] -> [1, 128, 20, 100, 100]
            #lidar_voxel = lidar_voxel.view(b, c*2, self.z_dim, self.y_dim, self.x_dim)
