...
class VFNet(nn.Module):
    ...
    def __init__(self, cfg, feat_in_dim, feat_out_dim, model='depth'):
        ...
        self.conv3d_block = Conv3DBlock(in_channels=128, out_channels=[128, 64], kernel_size=3, stride=1, padding=1)
    ...
    def project_voxel_into_image(self, cam_voxel_feat, lidar_voxel_feat, inv_K, extrinsics):
        ...
        '''
        # (b, c(64), depth_bin, h, w)
        depth_grid = self.depth_grid.view(self.batch_size, 3, self.proj_d_bins, self.img_h, self.img_w)
        depth_grid = depth_grid.permute(0, 2, 3, 4, 1).to('cuda')
        depth_feat = F.grid_sample(cam_voxel_feat, depth_grid, mode='bilinear', padding_mode='zeros', align_corners=True) 
        print(depth_feat.sum())
        depth_feat = self.conv3d_2(depth_feat)
        print(depth_feat.sum())
        depth_prob = F.softmax(depth_feat, dim=2)
        max_depth_prob, max_depth_idx = torch.max(depth_prob, dim=2, keepdim=True)  # (b, c, 1, h, w)
        voxel_grid = self.create_voxel_grid(self.voxel_str_p, self.voxel_end_p, self.voxel_size) # (b, 3, z, y, x)
        voxel_grid = voxel_grid.permute(0, 2, 3, 4, 1).to('cuda')
        depth_resampled = F.grid_sample(max_depth_prob, voxel_grid, mode='bilinear', padding_mode='zeros', align_corners=True)
        print(depth_resampled.sum(), '\n')
        '''
        ...
        for cam in range(self.num_cams):
            ...
            depth_feat = F.grid_sample(lidar_voxel_feat, grid, mode='bilinear', padding_mode='zeros', align_corners=True) 
            depth_feat = self.conv3d_2(depth_feat)
            #print(depth_feat[0,0,:,0,0])
            #channel = depth_feat.shape[1]
            #depth_feat = depth_feat.view(self.batch_size, channel*self.proj_d_bins, self.img_h, self.img_w)
            depth_prob = F.softmax(depth_feat, dim=2)
            # depth_prob 텐서에서 가장 높은 값과 해당 인덱스를 찾습니다.
            #depth_prob = depth_prob.view(self.batch_size, channel, self.proj_d_bins, self.img_h, self.img_w)
            #max_depth_prob, max_depth_idx = torch.max(depth_prob, dim=2, keepdim=True)  # (b, c, 1, h, w)
            #depth_resampled = F.grid_sample(max_depth_prob, voxel_grid, mode='bilinear', padding_mode='zeros', align_corners=True)
            #voxel_feat2 = depth_resampled * voxel_feat

            proj_feat = F.grid_sample(voxel_feat, grid, mode='bilinear', padding_mode='zeros', align_corners=True) # (b,c,bin,h,w), [1, 128, 50, 48, 80])
            proj_feat = depth_prob * proj_feat
    ...
    def forward(self, inputs, feats_agg, frame_ids):
        ...
        if self.model == 'depth':
            ...
            lidar_voxel_feat = self.spconvTensor(pc[0][0])
