...
class VFNet(nn.Module):
    ...
    def __init__(self, cfg, feat_in_dim, feat_out_dim, model='depth'):
        ...
        self.voxel_gen = PointToVoxel(vsize_xyz=[1, 1, 1.5],
                        coors_range_xyz=[-50, -50, -15, 50, 50, 15],
                        num_point_features=35,
                        max_num_voxels=200000,
                        max_num_points_per_voxel=5)
        #self.conv3d_1 = nn.Conv3d(in_channels=289, out_channels=512, kernel_size=3, padding=1, stride=1)
        #self.conv3d_2 = nn.Conv3d(in_channels=512, out_channels=256, kernel_size=3, padding=1, stride=1)
        self.conv3d_pose = Conv3DBlock(in_channels=292, out_channels=[512, 256], kernel_size=3, stride=1, padding=1)
        self.conv3d_depth = Conv3DBlock(in_channels=99, out_channels=[128, 64], kernel_size=3, stride=1, padding=1)

        #self.vfe = VoxelFeatureExtractor(num_input_features=35, num_filters=[64, 128], voxel_size=(1, 1, 1.5), pc_range=(-50, -50, -15, 49, 49, 13.5))
        ...
        if model == 'depth':
            ...
            #encoder_dims = self.proj_d_bins * (self.v_dim_o[-1] + self.lidar_dim)
            #encoder_dims = self.proj_d_bins * self.v_dim_o[-1]
            encoder_dims = self.proj_d_bins * 64
            stride = 1
            
        else:
            #encoder_dims = (feat_in_dim + 1 + self.lidar_dim)*self.z_dim
            encoder_dims = 256 * self.z_dim
            stride = 2
        
        self.spconv_net = SpconvNet(encoder_dims)
    ...
    def project_voxel_into_image(self, voxel_feat, inv_K, extrinsics):
        ...
        #voxel_feat = torch.cat((voxel_feat, self.sp_voxel), dim=1)
        ...
        for cam in range(self.num_cams):
            ...
            #proj_feat = proj_feat.view(b, self.proj_d_bins * (self.v_dim_o[-1] + self.lidar_dim), self.img_h, self.img_w)
    ...
    def forward(self, inputs, feats_agg):
        ...
        point = inputs['point_cloud'][0][0]
        range_img, range_img_coord = inputs['range_image'][0][0], inputs['range_image_coord'][0][0].long()
        range_img = range_img.unsqueeze(0)  # 배치 차원 추가
        range_img = range_img.unsqueeze(0)  # 채널 차원 추가

        '''
        range_img, range_img_coord = generate_range_img(point.detach().clone().cpu().numpy())
        
        # numpy 배열을 PyTorch 텐서로 변환
        range_img = torch.from_numpy(range_img).float().to('cuda')
        range_img = range_img.unsqueeze(0)  # 배치 차원 추가
        range_img = range_img.unsqueeze(0)  # 채널 차원 추가
        range_img_coord = torch.from_numpy(range_img_coord).long().to('cuda')

        save_image(range_img, 'range_img.png')
        save_image(dataloader_range, 'dataloader_range.png')
        '''
        ...
        # pose: ([1, 257, 200000]), depth: ([1, 64, 200000])
        camera_voxel_feat = self.backproject_into_voxel(feats_agg, mask, K, extrinsics_inv)
        

        #print("voxel_feat: ", voxel_feat.shape)
        #self.spconvTensor(lidar[0][0])

        range_feat = self.pfe(range_img)

        
        point = point.T.unsqueeze(0)  # 배치 차원 추가

        pointwise = torch.empty((1, 35, point.shape[0]))

        for i in range(point.shape[0]):
            r = range_img_coord[i][0]
            c = range_img_coord[i][1]
            # print(x_1a[:,:,r,c].shape) # (1, 32)
            # print(pointwise[:,:,i].shape) # (1, 3)
            concat_feat = torch.cat((point[:,:,i], range_feat[:,:,r,c]), axis=1) # (1, 35)
            # 결과 텐서에 저장
            pointwise[:,:,i] = concat_feat

        voxels, voxel_coords, num_points_in_voxel, pc_voxel_id = self.voxel_gen.generate_voxel_with_id(pointwise[0,:,:].T)
        voxels = voxels.to('cuda')
        voxelwise = self.vfe(voxels)
        lidar_voxel_feat = torch.zeros((1, 35, 200000))

        
        for i in range(voxels.shape[0]):
            x, y, z = voxel_coords[i][2], voxel_coords[i][1], voxel_coords[i][0]
            feat = voxelwise[i,0,:] # voxelwise feature
            idx = (z * (self.y_dim * self.x_dim)) + (y * self.x_dim) + x
            lidar_voxel_feat[:,:,idx] = feat # torch.Size([1, 35, 200000])
        
        if torch.isnan(lidar_voxel_feat).any():
            print("NaN")

        lidar_voxel_feat = torch.nan_to_num(lidar_voxel_feat, nan=0.0)
        fusion_feat2d = torch.cat((camera_voxel_feat, lidar_voxel_feat.to('cuda')), dim=1)
        fusion_feat3d = fusion_feat2d.view(1, fusion_feat2d.shape[1], self.z_dim, self.y_dim, self.x_dim)  # 복원된 3D 구조
        ...
        if self.model == 'depth':
            voxel_feat = self.conv3d_depth(fusion_feat3d)
            voxel_feat = voxel_feat.view(1, 64, self.z_dim * self.y_dim * self.x_dim)
            ...
            voxel_feat = self.conv3d_pose(fusion_feat3d)
            voxel_feat = voxel_feat.view(1, 256, self.z_dim * self.y_dim * self.x_dim)
