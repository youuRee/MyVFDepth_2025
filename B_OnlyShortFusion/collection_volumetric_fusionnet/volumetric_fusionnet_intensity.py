...
class VFNet(nn.Module):
    ...
    def __init__(self, cfg, feat_in_dim, feat_out_dim, model='depth'):
        ...
        self.conv3d_pose = Conv3DBlock(in_channels=260, out_channels=[512, 256], kernel_size=3, stride=1, padding=1)
        self.conv3d_depth = Conv3DBlock(in_channels=67, out_channels=[128, 64], kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, inputs, feats_agg, frame_ids):
        ...
        xyz = inputs['point_cloud'][0][0]
        intensity = inputs['intensity'][0][0]
    
        point = torch.cat((xyz,intensity), axis=1)
        ...
        voxels, voxel_coords, num_points_in_voxel, pc_voxel_id = self.voxel_gen.generate_voxel_with_id(point)
        
        lidar_voxel_feat = self.create_voxel_grid(self.voxel_str_p, self.voxel_end_p, self.voxel_size)
        prob_voxel = torch.zeros((1, 1, 200000))

        for i in range(voxels.shape[0]):
            z, y, x = voxel_coords[i]
            mean = voxels[i].mean(axis=0)
            lidar_voxel_feat[0,:,z,y,x] = mean[0:3]
            idx = (z * (self.y_dim * self.x_dim)) + (y * self.x_dim) + x
            prob_voxel[:,:,idx] = mean[3]
            
        # intensity 값을 반영하여 lidar_voxel_feat 업데이트
        lidar_voxel_feat = lidar_voxel_feat.view(1, 3, self.z_dim * self.y_dim * self.x_dim)
        # lidar_voxel_feat과 prob_voxel_3d를 곱하여 intensity를 반영
        lidar_voxel_feat *= prob_voxel
