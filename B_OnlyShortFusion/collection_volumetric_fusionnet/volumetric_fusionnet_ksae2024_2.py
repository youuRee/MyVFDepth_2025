...
class VFNet(nn.Module):
    ...
    def __init__(self, cfg, feat_in_dim, feat_out_dim, model='depth'):
        ...
        self.vfe = VoxelFeatureExtractor(num_input_features=3, num_filters=[8, 16], 
                                         voxel_size=(self.lidar_voxel_unit_size), 
                                         pc_range=(self.voxel_str_p + self.voxel_end_p))
        

        self.voxel_gen = PointToVoxel(vsize_xyz=self.lidar_voxel_unit_size,
                                coors_range_xyz=[-50, -50, 0, 50, 50, 20],
        ...
        self.conv3d_block = nn.Sequential(
            nn.Conv3d(16, 64, kernel_size=2, stride=2),  # 3x3x3 3D Convolution
            nn.MaxPool3d(kernel_size=2, stride=2),
            nn.BatchNorm3d(64),  # 3D Batch Normalization
            nn.ReLU()  # ReLU activation      
        )

        self.emptynet = SimpleMLP(input_dim=1,  output_dim=16)
        self.unknownnet = SimpleMLP(input_dim=1,  output_dim=16)
        self.save_cnt = 0
    ...
    # 여기에서 복셀 라벨 전처리 (SimpleOcc 참고)
    def LiDARFeatureExtractor(self, lidars):
        ...
        for batch in range(self.batch_size):
            lidar = lidars[batch]
            mask = ~torch.isnan(lidar).any(dim=1)  # NaN이 아닌 포인트를 선택
            lidar = lidar[mask]
            #print(lidar.shape)
            # 복셀 초기화
            voxel_label = torch.zeros([1, 80, 400, 400])
            voxel_feat = torch.ones([1, 16, 80, 400, 400]).to('cuda')

            voxels, coords, num_points_in_voxel, _ = voxel_gen.generate_voxel_with_id(lidar)
            voxelwise = self.vfe(voxels, num_points_in_voxel, coords)

            lidar_center = torch.from_numpy(np.array([0.0, 0.0, 0.0])).unsqueeze(0).to(torch.float32)
            pts_xyz = voxels.mean(dim=1).float().detach().to('cpu')
            #pts_xyz = coords.detach().to('cpu') + torch.tensor([0.125, 0.125, 0.125], dtype=torch.float32)
            vector = pts_xyz - lidar_center
            length = vector.norm(dim=1)
            norm_vector = vector / length[:, None]

            stride = 0.25 # unit voxel size
            N_step = 50 # 원점~끝점 사이의 샘플링 점 개수
            last_empty_point = pts_xyz - stride * norm_vector
            empty_vector = last_empty_point - lidar_center
            ...
            all_empty = pts_xyz_all[:, 1:-3, :].flatten(0, 1).to('cuda')
            
            _, empty_coords, _, _ = voxel_gen.generate_voxel_with_id(all_empty)
            del all_empty
            #empty_coords = empty_coords.to('cuda')
            
            v_z, v_y, v_x = empty_coords[:, 0].long(), empty_coords[:, 1].long(), empty_coords[:, 2].long()
            empty_feat = self.emptynet(torch.tensor([-1.0], device='cuda'))
            voxel_feat[:, :, v_z, v_y, v_x] = empty_feat.unsqueeze(0).unsqueeze(2).expand(1, 16, empty_coords.shape[0])
            voxel_label[:, v_z.detach().to('cpu'), v_y.detach().to('cpu'), v_x.detach().to('cpu')] = 1
            
            v_z, v_y, v_x = coords[:, 0].long(), coords[:, 1].long(), coords[:, 2].long()
            voxel_feat[:, :, v_z, v_y, v_x] = voxelwise.T.unsqueeze(0)  # 차있는 복셀 -> voxelwise 값
            voxel_label[:, v_z.detach().to('cpu'), v_y.detach().to('cpu'), v_x.detach().to('cpu')] = 1

            # unknown_idx를 벡터화된 방식으로 처리
            unknown_idx = torch.nonzero(voxel_label[0,:,:,:] == 0).to('cuda')
            ...
            if unknown_idx.shape[0] > 0:
                ...
                voxel_feat[:, :, z, y, x] = unknown_feat.unsqueeze(0).unsqueeze(2).expand(1, 16, unknown_idx.shape[0])
            ...
            '''
            # voxel occ debug (visualization)
            import open3d as o3d

            voxel_grid = self.create_voxel_grid([-50.0, -50.0, 0.0], [50.0, 50.0, 20.0], [400, 400, 80]).squeeze(0)
            voxel_grid = voxel_grid.numpy()#np.random.rand(3, 30, 100, 100)  # 임의의 포인트 좌표 [3, 30, 100, 100]
            voxel_label2 = voxel_label.detach().to('cpu').numpy()
            
            # 포인트 추출
            points = voxel_grid[0].reshape(3, -1).T  # [N, 3] 형식으로 변환
            labels = voxel_label2.reshape(-1)      # 라벨 [N] 형식으로 변환
            # unknown이 아닌 포인트만 선택
            
            mask = labels != 0                    # unknown 이 아닌 포인트에 대한 마스크 생성

            
            filtered_points = points[mask]        # unknown 제외한 포인트
            filtered_labels = labels[mask]        # unknown 제외한 라벨

            # 색상 설정
            colors = np.zeros((filtered_points.shape[0], 3))  # 색상 초기화

            # 색상 할당 (empty: 빨간색, occupancy: 파란색)
            colors[filtered_labels == 1] = [1, 0, 0]  # empty -> 빨간색
            colors[filtered_labels == 2] = [0, 0, 1]  # occupancy -> 파란색
            ...
            o3d.io.write_point_cloud(os.path.join('/workspace/debug/', '{}'.format(self.save_cnt) + ".ply"), pcd)
            self.save_cnt += 1
            '''
        # 배치 차원을 따라 합치기
        voxel_feats = torch.cat(voxel_feats, dim=0)
        voxel_feats = self.conv3d_block(voxel_feats)
    ...
    # 복셀 라벨 전처리한 파일 불러와서 따로 전처리 안함 -> PointToVoxel 필요 X
    def LiDARFeatureExtractor2(self, lidars, voxels_label):
        ...
        for batch in range(self.batch_size):
            ...
            #voxel_label = torch.zeros([1, 80, 400, 400])
            voxel_feat = torch.ones([1, 16, 80, 400, 400]).to('cuda')
            #check_label = torch.zeros([1, 80, 400, 400]).to('cuda')

            # empty
            empty_idx = torch.nonzero(voxels_label[batch,0,:,:,:] == 1)
            empty_feat = self.emptynet(torch.tensor([-1.0], device='cuda'))
            if empty_idx.shape[0] > 0:
                z, y, x = empty_idx[:, 0].long(), empty_idx[:, 1].long(), empty_idx[:, 2].long()
                voxel_feat[:, :, z, y, x] = empty_feat.unsqueeze(0).unsqueeze(2).expand(1, 16, empty_idx.shape[0])
                #check_label[:, z, y, x] = 1
            
            # occupancy
            voxels, coords, num_points_in_voxel, _ = self.voxel_gen.generate_voxel_with_id(lidar)
            voxelwise = self.vfe(voxels, num_points_in_voxel, coords)

            #print("prepro occ label: ", torch.nonzero(voxels_label[batch,0,:,:,:] == 2).shape)
            #print("not prepro occ label: ", coords.shape)

            # 벡터화된 방식으로 차있는 복셀을 한 번에 할당
            z, y, x = coords[:, 0].long(), coords[:, 1].long(), coords[:, 2].long()
            #check_label[:, v_z, v_y, v_x] = 2
            voxel_feat[:, :, z, y, x] = voxelwise.T.unsqueeze(0)  # 차있는 복셀 -> voxelwise 값

            # unknown
            unknown_idx = torch.nonzero(voxels_label[batch,0,:,:,:] == 0)#.to('cuda')
            unknown_feat = self.unknownnet(torch.tensor([1.0], device='cuda'))
            # 벡터화된 방식으로 unknown_feat 할당
            if unknown_idx.shape[0] > 0:
                z, y, x = unknown_idx[:, 0].long(), unknown_idx[:, 1].long(), unknown_idx[:, 2].long()
            ...
            voxel_grid = self.create_voxel_grid([-50.0, -50.0, 0.0], [50.0, 50.0, 20.0], [400, 400, 80]).squeeze(0)
            voxel_grid = voxel_grid.numpy()#np.random.rand(3, 30, 100, 100)  # 임의의 포인트 좌표 [3, 30, 100, 100]
            voxel_label2 = check_label.detach().to('cpu').numpy()
    ...
    def forward(self, inputs, feats_agg, frame_ids):
        ...
        if self.model == 'depth':
            ...
            voxel_label = inputs[('voxel', frame_ids)]
            lidar_voxel_feat = self.LiDARFeatureExtractor2(pc, voxel_label)
            ...
            cur_voxel_label = inputs[('voxel', frame_ids[0])]
            next_voxel_label = inputs[('voxel', frame_ids[1])]

            cur_voxel = self.LiDARFeatureExtractor2(cur_pc, cur_voxel_label) # [b, 64, 20, 100, 100]
            next_voxel = self.LiDARFeatureExtractor2(next_pc, next_voxel_label)
