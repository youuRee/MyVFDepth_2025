...
class VFNet(nn.Module):
    ...
    def project_voxel_into_image(self, cam_voxel_feat, lidar_voxel_feat, prob_voxel, inv_K, extrinsics):
        """
        ...
        bev_mean = bev_feat.mean(dim=1)[0].detach().cpu().numpy()
        plt.imshow(bev_mean, cmap='viridis')
        plt.savefig(os.path.join('/workspace/VFDepth/init_ones_bev/', '{}'.format(self.save_cnt) + ".png"))
        self.save_cnt += 1
        '''
        '''
        if self.mode != 'train':
            ...
            bev_mean = bev_feat.mean(dim=1)[0].detach().cpu().numpy()
            plt.imshow(bev_mean, cmap='viridis')
            path = os.path.join('/workspace/VFDepth/voxel_prob_bev/', '{}'.format(self.save_cnt) + ".png")
            plt.savefig(path)
            self.save_cnt += 1
        
        voxel_feat = self.conv3d_block(voxel_feat) # 128 -> 256 -> 128
        bev_feat = voxel_feat.mean(dim=2)
        bev_mean = bev_feat.mean(dim=1)[0].detach().cpu().numpy()
        plt.imshow(bev_mean, cmap='viridis')
        plt.savefig('/workspace/VFDepth/voxel_prob_conv_bev.png')
        
        
        voxel_feat = voxel_feat * prob_voxel
        bev_feat = voxel_feat.mean(dim=2)
        bev_mean = bev_feat.mean(dim=1)[0].detach().cpu().numpy()
        plt.imshow(bev_mean, cmap='viridis')
        plt.savefig('/workspace/VFDepth/voxel_prob_bev.png')
        '''
        proj_feats = []
        for cam in range(self.num_cams):
    ...
    def LiDARFeatureExtractor(self, lidar):
        ...
        if self.model == 'depth':
            prob_voxel = self.VoxelRayCasting(coords, grid_range, voxel_size)
        else:
            prob_voxel = None
        ...
        return output_dense_tensor, prob_voxel
    ...
    def forward(self, inputs, feats_agg, frame_ids):
        ...
        if self.model == 'depth':
            ...
            lidar_voxel_feat, prob_voxel = self.LiDARFeatureExtractor(pc[0][0])
            # for each pixel, collect voxel features -> output image feature     
            proj_feats = self.project_voxel_into_image(cam_voxel_feat, lidar_voxel_feat, prob_voxel, inv_K, extrinsics)
            ...
            if self.aug_depth:
                ...
                proj_feats = self.project_voxel_into_image(cam_voxel_feat, lidar_voxel_feat, prob_voxel, inv_K, inputs['extrinsics_aug'])
            ...
            if self.syn_visualize:
                def _get_proj_feat(inv_K, ang_x, ang_y, ang_z):
                    ...
                    proj_feats = self.project_voxel_into_image(cam_voxel_feat, lidar_voxel_feat, prob_voxel, inv_K, tform_mat @ extrinsics)
                    return proj_feats[0]
            ...
            cur_voxel, _  = self.LiDARFeatureExtractor(cur_pc[0][0]) # [1, 64, 20, 100, 100]
            next_voxel, _ = self.LiDARFeatureExtractor(next_pc[0][0])
