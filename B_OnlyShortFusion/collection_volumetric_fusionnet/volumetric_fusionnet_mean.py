...
class VFNet(nn.Module):
    ...
    def __init__(self, cfg, feat_in_dim, feat_out_dim, model='depth'):
        ...
        self.pt_encoder = nn.Sequential(
                        nn.BatchNorm1d(7),
                        VoxelFeatureExtraction(7, 32),        
                        VoxelFeatureExtraction(32, 64),
        )
        self.ptconv_encoder = VoxelFeatureExtractionKPconv(64, 64, n_centers=8)
        '''
    ...
    def spconvTensor(self, lidar):
        ...
        print('voxel : ', voxels.is_cuda())
        voxels_np = voxels_th.numpy()
        print('voxel np : ', voxels_np.is_cuda())
        
        # x['points']: (N*batch,7): relative xyz (on voxel center), xyzi
        # x['coordinates']: (N*batch,4): batch, voxel_index_x, voxel_index_y, voxel_index_z

        avg_point = torch.mean(voxels, dim=1) # avg(xyzi) in one voxel
        relative_xyz = torch.tensor(avg_point[:, 0:3] - torch.tensor([0.5, 0.5, 0.75]).to(device)) # avg - center
        features = torch.cat((relative_xyz, avg_point), dim=1)
        voxel_coordinates = coords.to(torch.int32).to(device)

        batch_indices = torch.zeros((features.shape[0], 1), dtype=torch.int32).to(device)
        input_indices = torch.cat((batch_indices, voxel_coordinates), dim=1)

        unq, unq_inv, _ = torch.unique(input_indices, return_inverse=True, return_counts=True, dim=0)
        unq = unq.type(torch.int32)

        point_wise = self.pt_encoder(features)
        voxel_wise_local = self.ptconv_encoder(features[:,0:3], point_wise, unq_inv)
        
        # 입력 특징 텐서와 인덱스 텐서를 디바이스로 이동
        #features_device = torch.tensor(features, dtype=torch.float32).to(device)
        # depth_device = torch.tensor(depth, dtype=torch.float32).to(device)
        #input_indices_device = torch.tensor(input_indices, dtype=torch.int32).to(device)

        # spconv 3D 희소 텐서 생성
        # indices는 spatial_shape 범위 안
        input_sp_tensor = spconv.SparseConvTensor(
            features=voxel_wise_local,
