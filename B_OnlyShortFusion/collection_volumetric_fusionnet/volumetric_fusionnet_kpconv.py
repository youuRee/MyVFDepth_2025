...
class VFNet(nn.Module):
    ...
    def spconvTensor(self, lidar):
        ...
        #avg_point = avg_point.unsqueeze(1).expand(-1, 5, -1)
        #relative_xyz = torch.tensor(voxels - avg_point[:, :]).to(device) # avg - center
        #features = torch.cat((voxels, relative_xyz), dim=2)
        features = voxels.view(voxels.shape[0], -1)
        ...
        # voxel_wise_local = self.ptconv_encoder(avg_point, point_wise, unq_inv)
