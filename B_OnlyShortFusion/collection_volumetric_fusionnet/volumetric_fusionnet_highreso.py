...
class VFNet(nn.Module):
    ...
    def __init__(self, cfg, feat_in_dim, feat_out_dim, model='depth'):
        ...
        self.vfe = VoxelFeatureExtractor(num_input_features=3, num_filters=[32, 64], voxel_size=self.voxel_unit_size, pc_range=self.voxel_str_p+self.voxel_end_p)
    ...
    def spconvTensor(self, lidar):
        ...
        voxel_gen = PointToVoxel(vsize_xyz=self.voxel_unit_size,
                                coors_range_xyz=self.voxel_str_p + self.voxel_end_p,
                                num_point_features=3,
                                max_num_voxels=self.n_voxels,
        ...
        input_sp_tensor = spconv.SparseConvTensor(
            ...
            spatial_shape=torch.tensor([self.z_dim, self.y_dim, self.x_dim]).to(device),
