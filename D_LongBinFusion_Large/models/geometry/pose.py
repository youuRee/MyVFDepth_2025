...
class Pose:
    ...
    def compute_pose(self, net, inputs):
        ...
        """
        if self.pose_model == 'fusion' or self.pose_model == 'gt':
    ...
    def get_single_pose(self, net, inputs ,cam):
        ...
        for f_i in self.frame_ids[1:]:
            ...
            if self.pose_model == 'fusion':
                ...
                output[('axisangle', 0, f_i)] = axisangle[:, 0]
                output[('translation', 0, f_i)] = translation[:, 0]
                output[('cam_T_cam', 0, f_i)] = vec_to_matrix(axisangle[:, 0], translation[:, 0], invert=(f_i < 0))    
            else:
                frame_ids = [0, -1] if f_i < 0 else [0, 1]
                output[('cam_T_cam', 0, f_i)], output[('axisangle', 0, f_i)], output[('translation', 0, f_i)] = net(inputs, frame_ids, cam)#vec_to_matrix(axisangle[:, 0], translation[:, 0], invert=(f_i < 0))            
        return output
    ...
    def distribute_pose(self, poses, exts, exts_inv):
        ...
        for f_i in self.frame_ids[1:]:
            ...
            ref_axisangle = poses.get(('axisangle', 0, f_i))  # ✅ 추가: axisangle 가져오기
            ref_translation = poses.get(('translation', 0, f_i))  # ✅ 추가: translation 가져오기
            ...
            for cam in range(self.num_cams):
                ...
                # ✅ 추가: 각 카메라에도 axisangle과 translation 저장
                if ref_axisangle is not None and ref_translation is not None:
                    outputs[('cam', cam)][('axisangle', 0, f_i)] = ref_axisangle
                    outputs[('cam', cam)][('translation', 0, f_i)] = ref_translation
