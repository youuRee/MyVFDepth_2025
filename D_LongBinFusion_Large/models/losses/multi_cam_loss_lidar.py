...
class MultiCamLoss(SingleCamLoss):
    ...
    def compute_lidar_loss_2d(self, inputs, target_view, cam=0, scale=0, ref_mask=None):
        ...
        for scale in self.scales:
            ...
            # single
            gt_depth = inputs[('gt_depth', 0)][:, cam, :, :]
            #gt_depth = target_view[('accum_depth', scale)]#[:, cam, :, :]
            pred_depth = target_view[('depth', scale)]
            #single_loss = F.l1_loss(pred_depth, gt_depth, reduction='mean')
    ...
    def compute_lidar_loss_instance(self, inputs, target_view, cam=0, scale=0, ref_mask=None):
        ...
        for scale in self.scales:
            ...
            gt_depth = inputs[('gt_depth', 0)][:, cam, :, :]
            pred_depth = target_view[('depth', scale)]
            pred_disp = target_view[('disp', scale)]
            
            mask = (gt_depth > min_depth) * (gt_depth < max_depth) * inputs['mask'][:, cam, ...]
            mask = mask.float()
            loss = F.l1_loss(pred_depth, gt_depth, reduction='none')
            sparse_depth_loss = (loss * mask).sum() / (mask.sum() + 1e-6)
            
            #seg_list = [d['segmentation'].astype(np.float32)[None, ...] for d in   # (1,H,W)
            #seg_mask = torch.from_numpy(np.stack(seg_list, axis=0)) 
            #seg_mask = seg_mask.to(device=gt_depth.device, dtype=gt_depth.dtype)
            
            seg_mask = inputs['sam_mask'][cam][0] # inputs['sam_mask'][cam] -> [B, N_inst, 1, 384, 640]
            seg_mask = seg_mask.to(device=pred_depth.device, dtype=pred_depth.dtype)
            
            if seg_mask.shape[0] == 0: # 아무런 인스턴스가 추출되지 않은 경우
                instancewise_loss = 0
            else:
                # pred_disp -> 근거리(큰 disp) 강조 가중치 만들기
                disp = pred_disp.detach()  # 가중치에 grad 안 흘리기
                # 지역 정규화: per-image min-max (너무 극단적이면 percentile로 대체 가능)
                dmin = torch.quantile(disp, 0.02)
                dmax = torch.quantile(disp, 0.98)
                w = (disp - dmin) / (dmax - dmin + 1e-6)
                w = w.clamp(0., 1.)

                # 선택적으로 감마로 근거리 강조 강도 조절 (e.g., gamma=2.0)
                gamma = 2.0
                w = (w ** gamma)

                # seg broadcast
                pred_e = pred_depth.expand_as(seg_mask)           # (N_inst,1,H,W)
                w_e    = w.expand_as(seg_mask)                    # (N_inst,1,H,W)

                # 가중 합/가중 분모
                wseg   = (w_e * seg_mask)                         # (N_inst,1,H,W)
                num    = (pred_e * wseg).sum(dim=(1,2,3))         # (N_inst,)
                den    = wseg.sum(dim=(1,2,3)).clamp_min(1e-6)    # (N_inst,)

                # 인스턴스별 '가중 평균 깊이'
                inst_pred_mean = num / den                        # (N_inst,)
                inst_mean_4d   = inst_pred_mean.view(-1,1,1,1)

                # '같은 가중치'로 편차 L1 계산
                inst_err_num = ( (pred_e - inst_mean_4d).abs() * wseg ).sum(dim=(1,2,3))  # (N_inst,)
                per_inst_loss = inst_err_num / den                                        # (N_inst,)

                instancewise_loss = per_inst_loss.mean()  # 스칼라
                '''
                # (1,1,H,W) -> (79,1,H,W) 브로드캐스트 (B=1이므로 가능)
                pred_e = pred_depth.expand_as(seg_mask)     # (79,1,H,W)
                weight = pred_disp.expand_as(seg_mask)

                # --- 인스턴스별 평균 GT 깊이 계산: (79,)
                # seg 내 픽셀들만의 평균 = sum(gt*seg) / sum(seg)
                seg_sum = seg_mask.sum(dim=(1,2,3)).clamp_min(1.0)             # (79,)
                inst_pred_sum = (pred_e * seg_mask).sum(dim=(1,2,3))           # (79,)
                inst_pred_mean = inst_pred_sum / seg_sum                       # (79,)

                # --- (79,) 평균을 (79,1,1,1)로 reshape하여 브로드캐스트
                inst_pred_mean_4d = inst_pred_mean.view(-1, 1, 1, 1)                  # (79,1,1,1)

                # --- 인스턴스 평균 기반 L1
                inst_err = weight * (pred_e - inst_pred_mean_4d).abs() * seg_mask           # (79,1,H,W)
                inst_num = inst_err.sum(dim=(1,2,3))                               # (79,)
                inst_den = seg_sum                                                 # (79,)
                per_inst_loss = inst_num / inst_den                                # (79,)
                instancewise_loss = per_inst_loss.mean()                           # 스칼라
                '''
            single_loss += 0.3*instancewise_loss + sparse_depth_loss
