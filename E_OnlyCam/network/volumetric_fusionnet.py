...
class VFNet(nn.Module):
    ...
    def __init__(self, cfg, feat_in_dim, feat_out_dim, model='depth'):
        ...
        # define a depth grid for short-range voxel projection
        ...
        # define long-range depth bins generated from camera features only
        self.long_proj_d_bins = getattr(self, 'long_proj_d_bins', self.proj_d_bins)
        self.long_proj_d_str = getattr(self, 'long_proj_d_str', self.proj_d_str)
        self.long_proj_d_end = getattr(self, 'long_proj_d_end', self.proj_d_end)
        ...
        if model == 'depth':
            ...
            encoder_dims = self.long_proj_d_bins * self.v_dim_o[-1]
            stride = 1
        ...
        if model == 'depth':
            # camera-only LongBinFusion layers. Short bins come from camera voxels,
            # and long bins are generated directly from per-camera image features.
            self.short_voxel_fusion_cam = nn.Conv3d(self.v_dim_o[-1], self.v_dim_o[-1], kernel_size=3, stride=1, padding=1)
            self.long_bin_fusion_cam = nn.Conv1d(self.v_dim_o[-1], self.v_dim_o[-1], kernel_size=1, stride=1, padding=0)
            self.bin_fusion_net = nn.Conv1d(self.v_dim_o[-1] * 2, self.v_dim_o[-1], kernel_size=1, stride=1, padding=0)
            self.depth_prob_net = nn.Conv2d(feat_in_dim, self.long_proj_d_bins + self.v_dim_o[-1], kernel_size=1, padding=0)
    ...
    def get_depth_feat(self, x):
        ...
        feat = x[:, self.long_proj_d_bins:(self.long_proj_d_bins + self.v_dim_o[-1])]
        return depth, depth.unsqueeze(1) * feat.unsqueeze(2)
    ...
    def project_voxel_into_image(self, feats_agg, voxel_feat, inv_K, extrinsics):
        """
        Project camera-only voxel features back into each image view.
        Short-range bins are sampled from the multi-camera voxel volume, while
        long-range bins are generated from each camera feature map.
        """
        b, feat_dim, _ = voxel_feat.size()
        voxel_feat = voxel_feat.view(b, feat_dim, self.z_dim, self.y_dim, self.x_dim)
        short_voxel = self.short_voxel_fusion_cam(voxel_feat)

        proj_feats = []
        for cam in range(self.num_cams):
            feats_img = feats_agg[:, cam, ...]
            _, cam_long_bin = self.get_depth_feat(feats_img)

            # construct short-range 3D ray samples for each image pixel
            ...
            short_bin = F.grid_sample(short_voxel, grid, mode='bilinear', padding_mode='zeros', align_corners=True)

            long_bin_flat = cam_long_bin.reshape(b, feat_dim, self.long_proj_d_bins * self.img_h * self.img_w)
            long_bin_flat = self.long_bin_fusion_cam(long_bin_flat)
            long_bin = long_bin_flat.view(b, feat_dim, self.long_proj_d_bins, self.img_h, self.img_w)

            long_bin_near = long_bin[:, :, :self.proj_d_bins, :, :]
            long_bin_far = long_bin[:, :, self.proj_d_bins:, :, :]

            fused_short_bin = torch.cat([short_bin, long_bin_near], dim=1)
            fused_short_bin = fused_short_bin.reshape(b, feat_dim * 2, self.proj_d_bins * self.img_h * self.img_w)
            fused_short_bin = self.bin_fusion_net(fused_short_bin)
            fused_short_bin = fused_short_bin.view(b, feat_dim, self.proj_d_bins, self.img_h, self.img_w)

            short_long_bin = torch.cat([fused_short_bin, long_bin_far], dim=2)
            proj_feat = short_long_bin.reshape(b, self.long_proj_d_bins * self.v_dim_o[-1], self.img_h, self.img_w)
    ...
    def forward(self, inputs, feats_agg):
        ...
        if self.model == 'depth':
            ...
            proj_feats = self.project_voxel_into_image(feats_agg, voxel_feat, inv_K, extrinsics)
            ...
            if self.aug_depth:
                ...
                proj_feats = self.project_voxel_into_image(feats_agg, voxel_feat, inv_K, inputs['extrinsics_aug'])
            ...
            if self.syn_visualize:
                def _get_proj_feat(inv_K, ang_x, ang_y, ang_z):
                    ...
                    proj_feats = self.project_voxel_into_image(feats_agg, voxel_feat, inv_K, tform_mat @ extrinsics)
                    return proj_feats[0]
