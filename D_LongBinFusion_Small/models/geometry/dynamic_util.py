...
class Project3D(nn.Module):
    ...
    def forward(self, points, K, T):
        ...
        # Normalize to image grid coordinates [-1, 1]
        pix_coords = cam_points[:, :2, :] / (cam_points[:, 2:3, :] + self.eps)  # (B, 2, H*W)
        pix_coords = pix_coords.view(self.batch_size, 2, self.height, self.width)
        pix_coords_norm = pix_coords.clone()
        pix_coords_norm[:, 0, :, :] /= self.width - 1
        pix_coords_norm[:, 1, :, :] /= self.height - 1
        pix_coords_norm = (pix_coords_norm - 0.5) * 2
        pix_coords_norm = pix_coords_norm.permute(0, 2, 3, 1)  # (B, H, W, 2)

        # ego_motion: 3D flow in (x,y,z)
        ego_motion = cam_points_3D[:, :3, :] - points[:, :3, :]  # (B, 3, H*W)

        # --- 추가 부분: 2D flow 계산 ---
        # 원래 투영된 픽셀 좌표
        orig_proj = torch.matmul(K[:, :3, :], points)  # (B, 3, H*W)
        orig_pix = orig_proj[:, :2, :] / (orig_proj[:, 2:3, :] + self.eps)  # (B, 2, H*W)

        # 이동 후 투영된 픽셀 좌표
        moved_proj = torch.matmul(K[:, :3, :], cam_points_3D)  # (B, 3, H*W)
        moved_pix = moved_proj[:, :2, :] / (moved_proj[:, 2:3, :] + self.eps)  # (B, 2, H*W)

        flow_2d = moved_pix - orig_pix  # (B, 2, H*W)
        flow_2d = flow_2d.view(self.batch_size, 2, self.height, self.width).permute(0, 2, 3, 1)  # (B, H, W, 2)

        return pix_coords_norm, ego_motion, flow_2d
