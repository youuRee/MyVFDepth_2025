...
class FusedDepthNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        #self.res_encoder = ResnetEncoder(self.num_layers, self.weights_init, 1) # number of layers, pretrained, number of input images
        #del self.res_encoder.encoder.fc
        
        self.encoder = MobileNetEncoder(pretrained=True, num_input_images=6)
        enc_feat_dim = sum(self.encoder.num_ch_enc[self.fusion_level:])
        self.conv1x1 = conv2d(self.encoder.num_ch_enc[self.fusion_level], self.fusion_feat_in_dim, kernel_size=1, padding_mode = 'reflect')
    ...
    def forward(self, inputs):
        ...
        sf_images = torch.stack([self.camli_conv2d(torch.cat([inputs[('color_aug', 0, 0)][:, cam, ...], inputs[('gt_depth', 0)][:, cam, :, :]], dim=1)) for cam in range(self.num_cams)], 1)
        ...
        #packed_feats_agg = self.conv1x1(torch.cat(packed_feats_list, dim=1))   # [6, 256, 48, 80]     
        #feats_agg = unpack_cam_feat(packed_feats_agg, self.batch_size, self.num_cams)
        
        feats_agg = self.conv1x1(packed_feats[lev])
        feats_agg = unpack_cam_feat(feats_agg, self.batch_size, self.num_cams)
        # fusion_net, backproject each feature into the 3D voxel space
        fusion_dict = self.fusion_net(inputs, feats_agg, 0)        
        ...
        #outputs['motion_loss'] = fusion_dict['motion_loss'] 
