...
class VFNet(nn.Module):
    ...
    def __init__(self, cfg, feat_in_dim, feat_out_dim, model='depth'):
        ...
        self.long_bin_fusion = nn.Conv1d(128, 64, kernel_size=1, stride=1, padding=0)
        
        self.bin_fusion_net = nn.Conv1d(128, 64, kernel_size=1, stride=1, padding=0)
    ...
    def project_voxel_into_image(self, feats_agg, cam_voxel_feat, lidar_voxel_feat, inv_K, extrinsics):
        ...
        for cam in range(self.num_cams):
            ...
            long_bin_flat = long_bin.view(b, feat_dim*2, self.long_proj_d_bins*self.img_h*self.img_w)
            long_bin_flat = self.long_bin_fusion(long_bin_flat) # [1, 64, 100, 48, 80]
            ...
            fused_short_bin_flat = fused_short_bin.view(b, feat_dim*2, self.proj_d_bins*self.img_h*self.img_w)
            fused_short_bin_flat = self.bin_fusion_net(fused_short_bin_flat) # (B, C'(64), 50, H, W)
            fused_short_bin = fused_short_bin_flat.view(b, feat_dim, self.proj_d_bins, self.img_h, self.img_w)
