...
class DDADdatasetSF(DGPDataset):
    ...
    def __getitem__(self, idx):
        ...
        slam_path = scene_dir.replace('raw_data', 'slam_output') + '/' + os.path.basename(scene_dir) + '_poses.npy'
        slam_pose = np.load(slam_path)
        cur_L_pose = slam_pose[sample_idx_in_scene]     #self.get_current('pose', 6).matrix
        prev_L_pose = slam_pose[sample_idx_in_scene-1]  #self.get_context('pose', 6)[0].matrix
        next_L_pose = slam_pose[sample_idx_in_scene+1]  #self.get_context('pose', 6)[1].matrix
        ...
        # sample['cur_empty'] = (6, N, 3)
        # sample['cur_voxel'] = (6, 1, 80, 400, 400)
        #sample[('voxel', 0)] = sample['cur_voxel'][0,:,:,:,:]
        #sample[('voxel', -1)] = sample['prev_voxel'][0,:,:,:,:]
        #sample[('voxel', 1)] = sample['next_voxel'][0,:,:,:,:]
        '''
        sample[('lidar', 0)] = sample['point_cloud'][0,:,:]
        #sample[('depth', 0)] = sample['sparse_depth']
        for idx2, frame in enumerate(contexts):
            #contex_point = sample['point_context'][idx2][0,:,:]
            #contex_point = self.pad_or_truncate(contex_point)
            sample[('lidar', frame)] = sample['point_context'][idx2][0,:,:]#self.pad_or_truncate(contex_point)
            
        
        for key in ['cur_depth', 'prev_depth', 'next_depth', 'prev_pose', 'next_pose', 'point_cloud', 'point_context']:
            if key in sample:
                del sample[key]
        '''
