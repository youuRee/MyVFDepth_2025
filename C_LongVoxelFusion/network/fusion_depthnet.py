...
class FusedDepthNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        #self.motion_decoder = MotionDecoder(self.fusion_level, num_ch_enc, num_ch_dec, self.scales, use_skips = False)
...
class MotionDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.level_in, -1, -1):
            ...
            x = self.convs[('upconv', i, 1)](x)
            if i in self.scales:
                outputs[('motion_prob', i)] = self.sigmoid(self.convs[('dispconv', i)](x))                
        return outputs