...
class VFNet(nn.Module):
    ...
    def __init__(self, cfg, feat_in_dim, feat_out_dim, model='depth'):
        ...
        self.sensor_fusion_net = nn.Conv3d(128, 64, kernel_size=3, stride=1, padding=1)
    ...
    def project_voxel_into_image(self, cam_voxel_feat, lidar_voxel_feat, inv_K, extrinsics):
        ...
        short_voxel = self.sensor_fusion_net(voxel_feat) # [1, 64, 20, 100, 100]
        ...
        for cam in range(self.num_cams):
            ...
            long_bin = F.grid_sample(long_voxel, grid2, mode='bilinear', padding_mode='zeros', align_corners=True) # (b,c,bin,h,w), [1, 64, 100, 48, 80])
            ...
            short_long_bin = torch.cat([fused_short_bin, long_bin_100], dim=2) # (B, 64, 50, H, W)
            ...
            # Cost Volume 2D Proj Visualization (Not Train, For Test)
            cam_points = torch.matmul(inv_K[:, cam, :3, :3], self.pixel_grid)
            cam_points = self.long_depth_grid * cam_points.view(self.batch_size, 3, 1, self.num_pix) # [b, 3, n_depthbins, n_pixels]
            cam_points = torch.cat([cam_points, self.long_pixel_ones], dim=1) # [b, 3, n_depthbins, n_pixels] [b, 1, n_depthbins, n_pixels] -> [b, 4, n_depthbins, n_pixels]
            cam_points = cam_points.view(self.batch_size, 4, -1) # [b, 4, n_depthbins * n_pixels]
            ...
            # 3D grid_sample [b, n_voxels, 3], value: (x, y, z) point
            grid3 = points.permute(0, 2, 1) 
            
            for i in range(3):
                
                v_length = self.long_voxel_end_p[i] - self.long_voxel_str_p[i]
                grid3[:, :, i] = (grid3[:, :, i] - self.long_voxel_str_p[i]) / v_length * 2. - 1.
                
            grid3 = grid3.view(self.batch_size, self.long_proj_d_bins, self.img_h, self.img_w, 3) # (B, 50, h, w, 3)
            cost_voxel = lidar_voxel_feat[1]
            cost_bin = F.grid_sample(cost_voxel, grid3, mode='bilinear', padding_mode='zeros', align_corners=True) # (b,c,bin,h,w), [1, 64, 100, 48, 80])
            
            cost_proj_feat = cost_bin.view(b, self.long_proj_d_bins * self.v_dim_o[-1], self.img_h, self.img_w) # [1, 64*100, 48, 80]
            cost_proj_feat = self.reduce_dim(cost_proj_feat)
            cost_proj_feats.append(cost_proj_feat)
            '''
            with torch.no_grad():
                # normalize to [0, 1] for visualization
                vis_feat = cost_proj_feat[0]  # [C, H, W]

                # 방법 1: 채널 평균 (motion likelihood heatmap처럼)
                avg_feat = vis_feat.mean(dim=0).cpu().numpy()  # [H, W]

                plt.figure(figsize=(10, 5))
                plt.imshow(avg_feat, cmap='hot')
                plt.axis('off')
                plt.title(f'Cost Volume Projection - Camera {cam}')
                plt.tight_layout()
                plt.savefig(f'cost_volume_cam{cam}.png')
                plt.close()
            '''
        return proj_feats, cost_proj_feats
    ...
    def LiDARFeatureExtractor(self, lidar, inputs):
        ...
        # 포워드 패스를 수행하여 복셀 특징을 얻기
        output_sp_tensor = self.spconv_net(input_sp_tensor, 'long')
        past_long_voxel_feat = output_sp_tensor.dense()
        cost_volume = torch.abs(long_voxel_feat - past_long_voxel_feat)
        
        # cost volume으로 attention weight 만들기
        # Step 1. Feature 차이로 cost_volume 구함
        #cost_volume = torch.abs(long_voxel_feat - past_long_voxel_feat)  # [B, C, Z, Y, X]

        # Step 2. Channel-wise 압축: motion magnitude map 만들기
        #motion_mag = self.attn_net(cost_volume)  # [B, 1, Z, Y, X] #cost_volume.mean(dim=1, keepdim=True)  # [B, 1, Z, Y, X]

        # Step 3. Normalize
        #motion_mag_norm = motion_mag / (motion_mag.max() + 1e-6)  # scale to [0,1]

        # Step 4. Sigmoid (or softmax): attention처럼 스무딩
        #attn_weight = torch.sigmoid(motion_mag_norm)  # [B, 1, Z, Y, X]

        # Step 5. 적용
        #long_voxel_feat = long_voxel_feat * attn_weight
        #voxel_feats.append(long_voxel_feat)
        
        
        # 1. feature residual (delta)
        #delta = (long_voxel_feat - past_long_voxel_feat).pow(2).mean(1, keepdim=True)  # [B, 1, Z, Y, X]
        # 2. normalize
        #delta_norm = delta / (delta.max() + 1e-6)
        # 3. loss 계산
        #motion_loss = F.mse_loss(attn_weight, delta_norm)
    ...
    def forward(self, inputs, feats_agg, frame_ids):
        ...
        if self.model == 'depth':
            ...
            proj_feats, cost_feats = self.project_voxel_into_image(cam_voxel_feat, lidar_voxel_feat, inv_K, extrinsics)
            fusion_dict['proj_feat'] = pack_cam_feat(torch.stack(proj_feats, 1))
            fusion_dict['cost_feat'] = pack_cam_feat(torch.stack(cost_feats, 1))
            
            # with view augmentation
            if self.aug_depth:
                # extrinsics
                #inputs['extrinsics_aug'] = self.augment_extrinsics(extrinsics)
                proj_feats, _ = self.project_voxel_into_image(cam_voxel_feat, lidar_voxel_feat, inv_K, inputs['extrinsics_aug'])
            ...
            fusion_dict['motion_loss'] = motion_loss
            return fusion_dict       
