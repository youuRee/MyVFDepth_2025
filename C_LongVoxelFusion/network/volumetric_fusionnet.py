...
class VFNet(nn.Module):
    ...
    def __init__(self, cfg, feat_in_dim, feat_out_dim, model='depth'):
        ...
        voxel_grid = self.create_voxel_grid(self.long_voxel_str_p, self.long_voxel_end_p, self.long_voxel_size)        
        ...
        self.long_voxel_fusion = nn.Conv3d(128, 64, kernel_size=3, stride=1, padding=1)
    ...
    def project_voxel_into_image(self, cam_voxel_feat, lidar_voxel_feat, inv_K, extrinsics):
        ...
        cam_voxel_long = cam_voxel_feat.view(b, feat_dim, self.z_dim, self.y_dim, self.x_dim)
        cam_voxel_short = cam_voxel_long[:,:,:,50:150,50:150]
        
        # lidar_voxel_feat = [short feat, long feat]
        short_voxel = torch.cat((cam_voxel_short, lidar_voxel_feat[0]), dim=1) # [1, 128, 20, 100, 100]
        short_voxel = self.short_voxel_fusion(short_voxel) # [1, 64, 20, 100, 100]
        
        long_voxel = torch.cat((cam_voxel_long, lidar_voxel_feat[1]), dim=1) # [1, 128, 20, 200, 200]
        long_voxel = self.long_voxel_fusion(long_voxel) # [1, 64, 20, 200, 200]
