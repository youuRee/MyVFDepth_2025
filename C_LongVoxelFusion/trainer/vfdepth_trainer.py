...
class VFDepthTrainer:
    ...
    def validate(self, model):
        ...
        for cam in range(6):
            ...
            # 1. GT 이미지 + 세그먼트 마스크 오버레이
            axs[0, 0].imshow(gt_img)
            axs[0, 0].set_title("GT Image + Seg Masks")
            axs[0, 0].axis('off')

            ax = axs[0, 0]  # 현재 subplot의 axis 객체
            ax.set_autoscale_on(False)
            seg_mask = inputs['sam_mask'][cam][0]

            for ann in seg_mask:  # ann: (1, H, W)
                m = ann[0].detach().cpu().numpy()  # (H, W)
                img = np.ones((m.shape[0], m.shape[1], 3))
                color_mask = np.random.random((1, 3)).tolist()[0]  # RGB 랜덤 색
                for i in range(3):
                    img[:, :, i] = color_mask[i]
                # alpha 채널로 마스크 투명도 지정 (0.35)
    ...
    def evaluate(self, model, vis_results=False):
        ...
        for batch_idx, inputs in enumerate(process):   
            ...
            #if forder_name not in scene_list:
            #    continue

            #count += 1
            #print("Scene: ", forder_name, " (", count, ")")  
